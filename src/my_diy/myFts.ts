...
export class MyFts implements IIndexlessFts {
  public searchPath = async (dir: string, query: string) => {
    const docs: IDocument[] = [];
    for (const path of glob.sync(`${dir}/*.md`)) {
      if (path.endsWith('md') || path.endsWith('txt') || path.endsWith('log'))
      {
        const text = fs.readFileSync(path, 'utf8');
        docs.push({path: path, text});
      }
    }
...
function countStemmed(docText: string, term: string) {
  ...
  if (term.endsWith('y')) {
    temp += count(docText, new RegExp(`\\b${term.slice(0, -1)}ies\\b`, 'gi'));
